...
def reverse_part(positions, current_position, length):
    final_position = current_position + length
    # if the list does not wrap around
    if final_position < len(positions):
        before = positions[:current_position]
        to_reverse = positions[current_position:final_position]
        after = positions[final_position:]

        return before + list(reversed(to_reverse)) + after
    
    # if the list does wrap around
    final_position = final_position % len(positions)
    to_reverse = positions[current_position:] + positions[:final_position] 
    original = positions[final_position:current_position]

    after_reverse = list(reversed(to_reverse))
    after_reverse_split_point = len(positions)-current_position
    a_part = after_reverse[after_reverse_split_point:]
    b_part = after_reverse[:after_reverse_split_point]
    return a_part + original + b_part

def apply_transformations(lengths, n_pos=256):
    positions = list(range(0, n_pos))
    current_position = 0
    skip_size = 0

    for length in lengths:
        positions = reverse_part(positions, current_position, length)
...
assert apply_transformations([3, 4, 1, 5], n_pos=5)[0] == 3
assert apply_transformations([3, 4, 1, 5], n_pos=5)[1] == 4

def solve_challenge_a(data):
    lengths = parse_data(data)
    new_positions = apply_transformations(lengths)
...
assert parse_data_b("1,2,3") == [49, 44, 50, 44, 51, 17, 31, 73, 47, 23]
...
def apply_transformations_b(lengths, n_pos=256):
    positions = list(range(0, n_pos))
    current_position = 0
    skip_size = 0
    ...
    for _ in range(64):
        for length in lengths:
            positions = reverse_part(positions, current_position, length)
            current_position = (current_position + length + skip_size) % n_pos
            skip_size += 1

    return positions

def reduce_sparse_hash(positions):
    assert len(positions) == 256

    blocks = [positions[i*16:(i+1)*16] for i in range(16)]
    dense_hash = [reduce(lambda x,y: x^y, b) for b in blocks]

    knot_hash = ""
    for nr in dense_hash:
        assert 0 <= nr <= 255
        knot_hash += '{:x}'.format(nr).zfill(2)

    return knot_hash

def solve_challenge_b(data):
    lengths = parse_data_b(data)
    sparse_hash = apply_transformations_b(lengths)
    knot_hash = reduce_sparse_hash(sparse_hash)

    return knot_hash

assert solve_challenge_b("")         == "a2582a3a0e66e6e86e3812dcb672a272"
assert solve_challenge_b("AoC 2017") == "33efeb34ea91902bb2f59c9920caa6cd"
assert solve_challenge_b("1,2,3")    == "3efbe78a8d82f29979031a4aa0b16a9d"
assert solve_challenge_b("1,2,4")    == "63960835bcdc130f0b66d7ff4f6a5a8e"
